...
"You must order and merge your reference coordinates before performing a search"
    ...
    if ( $self->{_search_file} ) {
        if ( $self->{_search_file} =~ /\.gz$/ ) {
            $SEARCH = new IO::Uncompress::Gunzip $self->{_search_file}
              || die(
"IO::Uncompress::Gunzip failed while opening $self->{_search_file} for searching:\n$GunzipError\n"
              );
        }
        else {
            open( $SEARCH, $self->{_search_file} )
              || croak "Can't open search file \"$self->{_search_file}\" ";
        }
    ...
    croak
"search_list or search_file argument must be set for search_list subroutine "
      if not $self->{_search_list};
    ref $self->{_search_list} eq "ARRAY"
      || die "_search_list argument must be a reference to an array ";
    my $spl = "\t";
    $spl = '[:-]' if $self->{_list_type} eq "regions";
    my ( $start, $stop ) = ( 1, 2 );    #fine for regions and bed files
    ( $start, $stop ) = ( 2, 3 )
      if $self->{_list_type} eq
    ...
    foreach my $search ( @{ $self->{_search_list} } ) {
        ...
        croak
"Minimum of $min fields must be specified for list type $self -> {_list_type} "
          if @srch < $min;
    ...
    if ( $_[0]->{_location} < 0 ) {
          ...
          my $region ( @{ $_[0]->{_merged}->[ $_[0]->{_location} ]->{info} } )
        {
            ...
            { #with no valid column value we'll try and find the region field ourself
                ...
                $self->{_chrom} =~ s/^chr//i;
                if (
                    uc $split[$off] eq uc $self->{_chrom}
                    and ( $self->{_start} <=
                        ( $split[ $off + $stop ] + $self->{_flanks} )
                        and $self->{_stop} >=
                        ( $split[ $off + $start ] - $self->{_flanks} ) )
                  )
                {
                    push( @ret, $region );
                }
                ...
                $self->{_chrom} =~ s/^chr//i;
                if (
                    uc $split[ $self->{_col} - 1 ] eq uc $self->{_chrom}
                    and (
                        (
                            $self->{_start} <= (
                                $split[ $self->{_col} - 1 + $stop ] +
                                  $self->{_flanks}
                            )
                        )
                        and (
                            $self->{_stop} >= (
                                $split[ $self->{_col} - 1 + $start ] -
                                  $self->{_flanks}
                            )
                        )
                    )
...
"You must order and merge your reference coordinates before performing a search"
    ...
    croak "chrom argument is required for locate method "
      if not $self->{_chrom};
    $self->{_chrom} =~ s/^chr//i;
    set_start( $self, $args{start} ) if defined $args{start};
    croak "start argument is required for locate method "
      if not defined $self->{_start};
    $self->{_start} =~ s/\,//g;
    croak "Start argument must be an integer" if $self->{_start} !~ /^\d+$/;
    set_stop( $self, $args{stop} ) if defined $args{stop};
    croak "stop argument is required for locate method "
      if not defined $self->{_stop};
    $self->{_stop} =~ s/\,//g;
    ...
    while ( $l <= $u ) {
        ...
        if (
            ...
            or ( uc $self->{_chrom} eq uc $self->{_merged}->[$i]->{chrom}
                and $self->{_stop} <=
                ( $self->{_merged}->[$i]->{start} - $self->{_flanks} ) )
          )
        {
            $u = $i - 1;
        }
            ...
            or ( uc $self->{_chrom} eq uc $self->{_merged}->[$i]->{chrom}
                and $self->{_start} >=
                ( $self->{_merged}->[$i]->{end} + $self->{_flanks} ) )
          )
        {
            $l = $i + 1;
        }
...
$obj -> locate(chrom => "chr1", start => 10000000, stop => 12000000);
