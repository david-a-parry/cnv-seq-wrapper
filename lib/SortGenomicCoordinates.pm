...
sub check_custom {
    ...
    foreach ( $self->{_col}, $self->{_start_col}, $self->{_stop_col} ) {
        croak
"col, custom_start and custom_stop arguments must be defined when using \"custom\" type mode"
          if not defined $_;
    }
    croak "col argument $self->{_col} must be an integer"
      if $self->{_col} !~ /^-*\d+$/;
    croak "custom_start argument ($self->{_start_col}) must be an integer"
      if $self->{_start_col} !~ /^-*\d+$/;
    croak "custom_stop argument ($self->{_stop_col}) must be an integer"
      if $self->{_stop_col} !~ /^-*\d+$/;
}
...
sub return_info {
    ...
    if ( $_[0]->{_location} < 0 ) {
        ...
        ( $start, $stop ) = ( 2, 3 )
          if $self->{_type} eq
          "gene";    #gene tables have strand field between chr and start
        ...
        {
            if ( $self->{_col} == 0 )
            { #with no valid column value we'll try and find the region field ourself
                my $off = 0;
                my @split = ( split /$spl/, $region );
                no warnings 'uninitialized';
                $off++
                  until $split[$off] =~ /chr[\dXYMUG][TLn\d_]*/i
                  or $off > $#split;
                croak("no valid region found in line $region ")
                  if $off > $#split;
                $split[$off] =~ s/^chr//i;
                $self->{_chrom} =~ s/^chr//i;
                if (    uc $split[$off] eq uc $self->{_chrom}
                    and $self->{_position} > $split[ $off + $start ]
                    and $self->{_position} <= uc $split[ $off + $stop ] )
                {
                    push( @ret, $region );
                }
            }
                ...
                $self->{_chrom} =~ s/^chr//i;
                if ( uc $split[ $self->{_col} - 1 ] eq uc $self->{_chrom}
                    and $self->{_position} >
                    $split[ $self->{_col} + $start - 1 ]
                    and $self->{_position} <=
                    uc $split[ $self->{_col} + $stop - 1 ] )
                {
                    push( @ret, $region );
                }
            }
...
"Error, region was located but no matching regions for current chromosome and position values exist "
          if ( not @ret );
        return @ret
          if defined(wantarray);    #allow scalar or array for return value
        carp "return_info method called in void context"
          ;    #complain if not sent to either scalar or array
    }
    else {
...
"You must order and merge your reference coordinates before performing a search"
    ...
    $self->{_chrom} =~ s/^chr//i;
    set_position( $self, $args{position} ) if defined $args{position};
    ...
    while ( $l <= $u ) {
        my $i = int( ( $u + $l ) / 2 );
        if (
            ( uc $self->{_chrom} lt uc $self->{_merged}->[$i]->{chrom} )
            or (    uc $self->{_chrom} eq uc $self->{_merged}->[$i]->{chrom}
          ...
          )
        {
            ...
            ( uc $self->{_chrom} gt uc $self->{_merged}->[$i]->{chrom} )
            or (    uc $self->{_chrom} eq uc $self->{_merged}->[$i]->{chrom}
                and $self->{_position} > $self->{_merged}->[$i]->{start}
                and $self->{_position} > $self->{_merged}->[$i]->{end} )
          )
        ...
        elsif ( uc $self->{_chrom} eq uc $self->{_merged}->[$i]->{chrom}
            and $self->{_position} >= $self->{_merged}->[$i]->{start}
            and $self->{_position} <= $self->{_merged}->[$i]->{end} )
        {
            $self->{_location} = $i;
...
"IO::Uncompress::Gunzip failed while opening $self->{_file} for sorting:\n$GunzipError\n"
        ...
        #WE SHOULD ADD A REGEX TO CHECK THAT THE REGION IS CORRECT FORMAT REALLY
                    ...
                    $off <= $#split
                      ? [
                        $_,                      $split[$off],
                        $split[ $off + $start ], $split[ $off + $stop ]
                      ]
                      : carp("no valid region found in line $_"
                      )    #only keep lines with a valid chr field
                  } grep { !/^#/ } (<$FILE>)
            );
        }
        else {
            chomp(
                @sorted =
                  map { $_->[0] } #otherwise use user supplied column (using subtraction of 1 to convert to 0-based)
                  sort {
                         uc $a->[1] cmp uc $b->[1]
                      || $a->[2] <=> $b->[2]
                      || $a->[3] <=> $b->[3]
...
"An ordered array must be specified either by using \"->order\" method or by providing a reference to an array when calling \"->merge\" method "
    ...
    foreach my $table (@$merging) {
        ...
        { #with no valid column value we'll try and find the region field ourself
            no warnings 'uninitialized';
            $off++
              until $regions[$off] =~ /chr*[\dXYMUG][TnL\d_]*/i
              or $off > $#regions;
        ...
        croak "Array passed to merge subroutine is not sorted properly "
          ...
          if ( uc $cur_chrom lt uc $prev_chrom )
          or ( uc $cur_chrom eq uc $prev_chrom and $cur_start < $prev_start )
          and $count > 0;
        if ( $cur_start > $cur_end ) {
            carp
"Start value is after end value for region $cur_chrom:$cur_start-$cur_end. Swapping start and end values. ";
            $cur_end   = $regions[ $off + $start ];
            $cur_start = $regions[ $off + $stop ];

        }

        if (   ( uc $cur_chrom eq uc $prev_chrom and $cur_start > $prev_end )
            or ( uc $cur_chrom ne uc $prev_chrom ) )
        {
            my $hash = {
                chrom => $prev_chrom,
                start => $prev_start,
        ...
        elsif ( uc $cur_chrom eq uc $prev_chrom
            and $cur_start <= $prev_end
            and $cur_end > $prev_end )
